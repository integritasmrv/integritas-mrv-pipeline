...
DITTOFEED_URL = os.environ.get("DITTOFEED_URL", "http://144.91.126.111:3210")
DITTOFEED_WRITE_KEY = os.environ.get("DITTOFEED_WRITE_KEY", "")
...
async def identify_contact_to_dittofeed(email: str, first_name: str = "", last_name: str = "", company: str = "", phone: str = "", source: str = "cf7-lead") -> bool:
    if not DITTOFEED_WRITE_KEY:
        log.warning("Dittofeed write key not configured, skipping identify")
        return False
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(15)) as client:
            response = await client.post(
                f"{DITTOFEED_URL}/identify",
                headers={
                    "Authorization": f"Basic {DITTOFEED_WRITE_KEY}",
                    "Content-Type": "application/json",
                },
                json={"userId": user_id, "traits": traits},
            )
            if response.status_code in (200, 201, 202):
                log.info("Dittofeed identify success: %s", email)
                return True
            log.error("Dittofeed identify failed: %s %s", response.status_code, response.text[:200])
            return False
    except Exception as exc:
        log.error("Dittofeed identify error: %s", exc)
        return False


async def track_event_to_dittofeed(email: str, event_name: str, properties: dict | None = None) -> bool:
    if not DITTOFEED_WRITE_KEY:
        log.warning("Dittofeed write key not configured, skipping track")
        return False
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(15)) as client:
            ...
            if response.status_code in (200, 201, 202):
                log.info("Dittofeed track success: %s -> %s", email, event_name)
                return True
            log.error("Dittofeed track failed: %s %s", response.status_code, response.text[:200])
            return False
    except Exception as exc:
        log.error("Dittofeed track error: %s", exc)
        return False
...
async def process_lead(input: LeadInput, ctx: Context) -> LeadOutput:
    ...
    if email and "@" in email:
        await identify_contact_to_dittofeed(
            email=email,
        ...
        await track_event_to_dittofeed(
            email=email,
            event_name="cf7_lead_submitted",
            properties={
                "form_name": "contact",
                "message": parsed["message"][:500] if parsed["message"] else "",
                "company": parsed["company"],
            },
        )
        log.info("Dittofeed identify + track done for: %s", email)
    else:
        log.warning("Invalid email, skipping Dittofeed: %s", email)
    
    return LeadOutput(result=f"CF7 lead processed: {email}", lead_id=None)
...
NOCOBASE_DSN = os.environ.get(
    "NOCOBASE_DSN",
    "postgresql://noco_user:noco_pass@10.0.16.2:5432/nocobase",
)

async def write_mail_activity(
    email: str,
    event: str,
    ...
    conn = await asyncpg.connect(dsn=NOCOBASE_DSN, timeout=30)
    try:
        await conn.execute(
            """
            INSERT INTO mail_activity (email, event, campaign_id, template_id, subject, sender, tags, created_at)
        ...
        log.info("Wrote mail_activity: %s %s", email, event)
        return True
    except Exception as e:
        log.error("Failed to write mail_activity: %s", e)
        return False
    finally:
        await conn.close()


class BrevoEmailEventInput(BaseModel):
    email: str
    event: str
    campaign_id: str | None = None
    template_id: str | None = None
...
class BrevoEmailEventOutput(BaseModel):
    result: str
    mail_activity_written: bool


brevo_wf = hatchet.workflow(
    name="brevo-email-events",
    input_validator=BrevoEmailEventInput,
    on_events=["brevo:email_event"],
)


@brevo_wf.task(name="handle-brevo-email-event")
async def handle_brevo_email_event(input: BrevoEmailEventInput, ctx: Context) -> BrevoEmailEventOutput:
    log.info("Brevo email event: %s -> %s (campaign=%s)", input.email, input.event, input.campaign_id)

    written = await write_mail_activity(
        email=input.email,
        event=input.event,
    ...
    return BrevoEmailEventOutput(
        result=f"Brevo event processed: {input.event}",
        mail_activity_written=written,
    )


worker = hatchet.worker("integritas-worker", workflows=[cf7_wf, enr_wf, hs_wf, contact_wf, brevo_wf])

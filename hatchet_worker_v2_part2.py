...
async def nominatim_collect(entity_id: str, entity_label: str, primary: dict, round_n: int) -> list[dict]:
    ...
    return [{"entity_id": entity_id, "collector_name": "nominatim_geocode", "content_type": "json",
            "raw_content": json.dumps(structured)[:20000], "source_url": NOMINATIM_URL,
            "source_weight": 0.95, "round_number": round_n}]
...
async def overpass_collect(entity_id: str, entity_label: str, primary: dict, round_n: int) -> list[dict]:
    ...
    for el in elements:
        ...
        if not phone and not website:
            continue
        item = {"osm_id": el.get("id"), "osm_type": el.get("type"),
    ...
    return [{"entity_id": entity_id, "collector_name": "overpass_lookup", "content_type": "json",
            "raw_content": json.dumps(structured)[:20000], "source_url": OVERPASS_URL,
            "source_weight": 0.75, "round_number": round_n}]
...
async def scrapiq_collect(entity_id: str, entity_label: str, business_key: str, round_n: int) -> list[dict]:
    ...
    try:
        ...
        return [{"entity_id": entity_id, "collector_name": r["collector_name"],
                 "content_type": r["content_type"],
                 "raw_content": r["raw_content"][:20000],
                 "source_url": r["source_url"] or "",
                 "source_weight": 0.7, "round_number": round_n} for r in rows]
    finally:
        await conn.close()
...
async def social_analyzer_collect(entity_id: str, entity_label: str, round_n: int) -> list[dict]:
    ...
    if not profiles:
        return []
    return [{"entity_id": entity_id, "collector_name": "social_analyzer_api", "content_type": "json",
            "raw_content": json.dumps({"profiles": profiles})[:20000],
            "source_url": SOCIAL_ANALYZER_URL, "source_weight": 0.7, "round_number": round_n}]

#!/usr/bin/env python3
"""Patch webhook_hubspot handler in api_main.py"""
import subprocess
...
print(f"Found handler from {start_idx} to {end_idx}")
        ...
        for evt in events:
            try:
                ...
                if "company" in ot.lower():
                    ...
                    full = f"{fn} {ln}".strip() or (email.split("@")[0] if email else "Unknown")
...
with open('/tmp/api_main_patched.py', 'w') as f:
    f.write(new_content)

subprocess.run(['docker', 'exec', '-i', 'integritasmrv-api', 'python3', '-c',
    f'open("/app/api_main.py","w").write(open("/tmp/api_main_patched.py").read())'], check=True)

subprocess.run(['docker', 'restart', 'integritasmrv-api'], check=False)
